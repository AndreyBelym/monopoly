...
namespace gcn
{
    ...
    void UTF8TextField::keyPressed(KeyEvent & keyEvent)
    {
        ...
        {
            mCaretPosition = mStringEditor->prevCharacter(mText, mCaretPosition);
        }
        else if (key.getValue() == Key::RIGHT && mCaretPosition < mText.size())
        {
            mCaretPosition = mStringEditor->nextCharacter(mText, mCaretPosition);
        }
        else if (key.getValue() == Key::DELETE && mCaretPosition < mText.size())
        {
            mCaretPosition = mStringEditor->eraseCharacter(mText, mCaretPosition);
        }
        ...
        {
            mCaretPosition = mStringEditor->insertCharacter(mText, mCaretPosition, key.getValue());
        }
